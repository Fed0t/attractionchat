...
const htmlRegex = new RegExp(/<div class="arrowchat_image_message">[\s\S\d]*<\/div>/i);
...
const SingleMessage = (props) => {
    ...
    if (props.images.length > 0) {
        ...
        if (imageParse.constructor === Array) {
            imageParse.forEach((image) => {
                imagesSources.push({
                    src: 'https://app.attractionclub.ro/storage' + image.path + '/' + image.id,
                    thumbnail: 'https://app.attractionclub.ro/storage' + image.path + '/thumbnail_' + image.id
                });
            });
        } else {
            imagesSources.push({src: 'https://app.attractionclub.ro/storage' + imageParse.path + '/' + imageParse.id});
        }
        imageSource = <Gallery
    ...
    if (props.location.length > 0) {
        ...
        mapsLink = (<span style={{marginLeft: 5}}>
           <a rel="noopener noreferrer" target="_blank" style={{color: 'red'}}
              href={'http://maps.google.com/?q=' + locationObject.lat + ',' + locationObject.long}>
               Ti-am trimis un mesaj cu locatia mea in momentul acesta.

            </a>
            <div style={{padding: 5}}>
                 <iframe title="map" width="100%" height="180" frameBorder="0"
                         src={"https://www.bing.com/maps/embed?&cp=" + locationObject.lat + "~" + locationObject.long + "&lvl=16&typ=d&sty=r"}
                         scrolling="no">
                </iframe>
            </div>
        </span>);
    ...
    if (privatePhotosRegex.test(props.text)) {
        ...
        privatePhotos.private_photos.forEach((photo) => {
            imagesGrid.push({
                src: 'https://attractionclub.ro/assets/cache/images/images-full-' + photo + '.jpg',
                thumbnail: 'https://attractionclub.ro/assets/cache/images/images-small-' + photo + '.jpg',
            });
        });
        message = (
            <Gallery subheading={props.user.username + ' ti-a trimis galeria privata din aplicatie.'}
                     images={imagesGrid}/>
        )
    }
    ...
    if (publicPhotosRegex.test(props.text)) {
        ...
        message = (
            <Gallery subheading={props.user.username + ' ti-a trimis galeria publica din aplicatie.'}
                     images={imagesGrid}/>
        )
    }
    ...
    if (oldPhotos.test(props.text)) {
        ...
        message = (
            <Gallery images={[{
                src: 'https://attractionclub.ro/chat/public/download.php?file='+srcWithQuotes+'_t',
                thumbnail: 'https://attractionclub.ro/chat/public/download.php?file='+srcWithQuotes+'_t'
            }]}/>
        )
    }

    return (
        <div className="blockquote-box-mesaj clearfix" title={moment.unix(props.timestamp).format('HH:mm')}
             style={{marginTop: 15, marginBottom: 10}}>
            {(props.isRight) ? (
                    <div className="pull-right right-message">
                        <div className="h5-msg">
                            {message}
                        </div>
                        {mapsLink}
                        {imageSource}
